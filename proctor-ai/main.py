...
# #             if vad.is_speech(frame.tobytes(), sample_rate=samplerate):
# #                 is_speech = True
# #                 break

# #         violation_type = "voice_activity_detected" if is_speech else None
...
#             frame = data[start:start + frame_size].tobytes()
...
# Add FFmpeg to PATH before importing AudioSegment
...
def analyze_audio_clip(clip: AudioClip):
    try:
        ...
        consecutive_speech_required = 3  # number of consecutive frames to trigger speech
        speech_count = 0
        speech_detected = False
