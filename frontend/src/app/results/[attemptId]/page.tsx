...
const ScoreCircle = ({ percentage }: { percentage: number }) => {
  ...
  return (
    <div className="relative" style={{ width: sqSize, height: sqSize }}>
      <svg width={sqSize} height={sqSize} viewBox={viewBox}>
        <circle className="text-gray-700/50" cx={sqSize / 2} cy={sqSize / 2} r={radius} strokeWidth={`${strokeWidth}px`} stroke="currentColor" fill="none" />
        <circle cx={sqSize / 2} cy={sqSize / 2} r={radius} strokeWidth={`${strokeWidth}px`} transform={`rotate(-90 ${sqSize / 2} ${sqSize / 2})`} style={{ strokeDasharray: dashArray, strokeDashoffset: dashOffset, stroke: strokeColor }} strokeLinecap="round" fill="none" className="transition-all duration-1000 ease-in-out" />
      </svg>
      <div className="absolute inset-0 flex flex-col items-center justify-center">
...
const GlobalStyles = () => (
  <style jsx global>{`
    ...
    @keyframes backgroundPan { 0% { background-position: 0% 50%; } 50% { background-position: 100% 50%; } 100% { background-position: 0% 50%; } }
    .animate-fadeInUp { animation: fadeInUp 0.6s ease-out forwards; }
    .bg-sentinel-animated { background: linear-gradient(90deg, #020617, #082f49, #020617); background-size: 200% 200%; animation: backgroundPan 15s ease infinite; }
  `}</style>
);
...
export default function ResultPage({ params }: PageProps) {
  ...
  const formatLogMessage = (logType: string) => {
    switch (logType) {
      case 'tab_switch': return { message: 'Navigated away from the test tab.', level: 'Suspicious', icon: AlertTriangleIcon };
      case 'no_face_detected': return { message: 'User was not detected in the camera frame.', level: 'Fatal', icon: XCircleIcon };
      case 'multiple_faces_detected': return { message: 'Multiple people were detected in the camera frame.', level: 'Fatal', icon: XCircleIcon };
      case 'voice_activity_detected': return { message: 'Voice or speech was detected during the test.', level: 'Fatal', icon: XCircleIcon };
      default: return { message: 'An unknown event was logged.', level: 'Info', icon: () => null };
    }
  };
  ...
  useEffect(() => {
    ...
    const fetchResult = async () => {
      const token = localStorage.getItem('token');
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/results/${attemptId}`, { headers: { Authorization: `Bearer ${token}` } });
        if (!res.ok) throw new Error('Could not fetch results.');
        const data = await res.json();
        setResult(data);
      } catch (error) {
  ...
  const handleComplaintSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error('Failed to submit complaint.');
      setSubmissionStatus('success');
      setTimeout(() => { setIsModalOpen(false); setSubmissionStatus('idle'); setComplaintMessage(''); }, 2000);
    } catch {
      setSubmissionStatus('error');
    }
  };
      ...
      <div className="min-h-screen bg-sentinel-animated font-sans text-white p-4 sm:p-8">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="bg-black/70 border border-gray-800 rounded-2xl shadow-2xl p-8 backdrop-blur-xl animate-fadeInUp" style={{ animationDelay: '300ms' }}>
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-2xl font-bold text-white">Proctoring Report</h2>
              <p className="text-sm font-semibold bg-red-900/50 text-red-400 border border-red-700/50 px-3 py-1 rounded-full">Fatal Strikes: {result.fatalStrikes} / 3</p>
            </div>
            {result.proctoringLogs.length === 0 ? (
              <div className="text-center py-12"><ShieldCheckIcon /><p className="text-green-400 font-semibold mt-4 text-lg">System Integrity Nominal</p><p className="text-gray-400 text-sm">No suspicious activity was detected during the session.</p></div>
            ) : (
              <div className="space-y-3">
                {result.proctoringLogs.map((log) => {
                  const { message, level, icon: LogIcon } = formatLogMessage(log.type);
                  ...
                  const isSuspicious = level === 'Suspicious';
                  return (
                    <div key={log._id} className={`bg-gray-900/70 p-4 rounded-lg border-l-4 flex justify-between items-center transition-all hover:bg-gray-800/50 ${isFatal ? 'border-red-500' : isSuspicious ? 'border-yellow-500' : 'border-gray-600'}`}>
                      <div className="flex items-center">
                        <div className={`mr-4 w-10 h-10 rounded-full flex-shrink-0 flex items-center justify-center ${isFatal ? 'bg-red-500/10' : 'bg-yellow-500/10'}`}><LogIcon /></div>
                        <div><p className={`font-semibold ${isFatal ? 'text-red-400' : isSuspicious ? 'text-yellow-400' : 'text-gray-300'}`}>{level} Event</p><p className="text-gray-300">{message}</p></div>
                      </div>
                      <p className="text-sm text-gray-500 font-mono">{new Date(log.timestamp).toLocaleTimeString()}</p>
                    </div>
                  );
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mt-8 animate-fadeInUp" style={{ animationDelay: '500ms' }}>
            <button onClick={() => router.push('/student')} className="bg-gradient-to-r from-cyan-500 to-blue-600 text-white py-3 px-12 rounded-lg font-bold hover:from-cyan-400 hover:to-blue-500 transition-all transform hover:scale-105 hover:shadow-lg hover:shadow-cyan-500/20">Back to Dashboard</button>
            <button onClick={() => setIsModalOpen(true)} className="bg-gray-700 text-white py-3 px-12 rounded-lg font-bold hover:bg-gray-600 transition-all transform hover:scale-105">Report an Issue</button>
          </div>
        </div>
      </div>
      {isModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4">
          <div className="bg-white text-gray-800 rounded-lg shadow-2xl p-8 max-w-lg w-full">
            <h2 className="text-2xl font-bold mb-4">Submit a Complaint</h2>
            {submissionStatus === 'success' ? (
              <div className="text-center py-10"><p className="text-xl font-semibold text-green-600">Complaint Submitted Successfully!</p><p className="text-gray-600">Our team will review your issue shortly.</p></div>
            ) : (
              <form onSubmit={handleComplaintSubmit}>
                <p className="text-gray-600 mb-4">Please describe the issue you experienced during the test "{result.testId.title}".</p>
                <textarea value={complaintMessage} onChange={(e) => setComplaintMessage(e.target.value)} required rows={5} className="w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500" placeholder="e.g., Question #5 had a typo, a proctoring violation was flagged incorrectly..." />
                {submissionStatus === 'error' && <p className="text-red-500 mt-2 text-sm">Failed to submit. Please try again.</p>}
                <div className="flex justify-end gap-4 mt-6">
                  <button type="button" onClick={() => setIsModalOpen(false)} className="py-2 px-6 text-gray-700 bg-gray-200 rounded-lg hover:bg-gray-300">Cancel</button>
                  <button type="submit" disabled={submissionStatus === 'submitting'} className="py-2 px-6 text-white bg-blue-600 rounded-lg hover:bg-blue-700 disabled:bg-blue-400">{submissionStatus === 'submitting' ? 'Submitting...' : 'Submit Complaint'}</button>
                </div>
              </form>
            )}
          </div>
